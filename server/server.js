...
async function ttsFile(text, voice, speed, log) {
  ...
  const res = await fetch(EL_BASE + '/v1/text-to-speech/' + encodeURIComponent(voice), {
    ...
    body: JSON.stringify({
      text,
      model_id: 'eleven_multilingual_v2',  // Model terbaik untuk Indonesia & multi-bahasa
      voice_settings: {
        stability: 0.60,          // Lebih stabil, suara konsisten
        similarity_boost: 0.90,   // Lebih jernih dan jelas
        style: 0.40,              // Sedikit ekspresif tapi tidak berlebihan
        use_speaker_boost: true   // Volume stabil & terdengar jelas
      }
    })
  });
  if (!res.ok) {
